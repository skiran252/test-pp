// background/messages/predict.ts
...
async function loadModels() {
  console.time('Model Loading (Worker)');
  ...
  console.timeEnd('Model Loading (Worker)');
}
...
async function runPrediction(frameData: { data: Uint8ClampedArray | Uint8Array; width: number; height: number; }): Promise<string | null> {
  if (!processor) {
    await loadModels();
  }
  ...
  console.time('Total Inference Time (Worker)');
  ...
  try {
    ...
    console.timeEnd('Total Inference Time (Worker)');
    return prediction;
...
const handler: PlasmoMessaging.MessageHandler = async (req, res) => {
  ...
  const prediction = await runPrediction(frameData);
  res.send({
